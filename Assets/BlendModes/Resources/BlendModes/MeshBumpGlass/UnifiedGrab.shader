...
Shader "BlendModes/MeshBumpGlass/UnifiedGrab" {
Properties {
...
float4 _BMMeshSharedGT_TexelSize;
sampler2D _BumpMap;
sampler2D _MainTex;
...
half4 frag( v2f i ) : COLOR
{
	...
	float2 offset = bump * _BumpAmt * _BMMeshSharedGT_TexelSize.xy;
	i.uvgrab.xy = offset * i.uvgrab.z + i.uvgrab.xy;
	
	half4 col = tex2Dproj( _BMMeshSharedGT, UNITY_PROJ_COORD(i.uvgrab));
	half4 tint = tex2D( _MainTex, i.uvmain );
	
	fixed4 grabColor = col;
	fixed4 color = tint;
